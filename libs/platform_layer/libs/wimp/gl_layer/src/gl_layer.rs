...
use crate::gl_layer_types::{DEPTH_MIN, DEPTH_MAX, Vertex};
...
use crate::gl_layer_types::{Vertex, extract_tex_coords, TexCoords, set_full_alpha, TextOrRect};
...
impl <'font> State<'font> {
    ...
    where
        for <'r> Update: FnMut(glyph_brush::rusttype::Rect<u32>, &'r [u8]) + Copy,
...
pub fn render(
    ...
    let brush_action = state.text_rendering.render_vertices(
        text_or_rects,
        (width, height),
        |rect: glyph_brush::rusttype::Rect<u32>, tex_data: &_| {
