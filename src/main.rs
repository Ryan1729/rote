use macros::d;
use platform_types::{EditorAPI, Input, UpdateAndRenderOutput};
...
fn state_call<In, Out>(r#in: In, callback: fn(&mut State, In) -> Out) -> Out {
...
fn load_buffer_view(buffer_name: BufferName) -> BufferView {
    state_call(input, editor::load_buffer_view)
}
